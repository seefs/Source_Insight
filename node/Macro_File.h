
// File List [Num][Ca]:
// 1. 常用笔记
Save:SI\node\Macro_File.h \[1.1\] work--node
Save:SI\node\Macro_File.h \[1.2\] Tmp
// 2. 未整理 未改
Save:SI\node\Macro_File.h \[2.2\] 未整理 未改
Save:SI\node\Macro_File.h \[2.3\] bak文件:
// 3. Cmd
Save:SI\node\Macro_File.h \[3.1\] Cmd:
Save:SI\node\Macro_File.h \[3.2\] Cmd unzip:
Save:SI\node\Macro_File.h \[3.3\] Cmd 对比:
Save:SI\node\Macro_File.h \[3.4\] Macro path:
// 4. Test
Save:SI\node\Macro_File.h \[4.1\] Test:
Save:SI\node\Macro_File.h \[4.2\] file list Test:
Save:SI\node\Macro_File.h \[4.3\] Msg
// 5. rule
Save:SI\node\Macro_File.h \[5.1\] rule
Save:SI\node\Macro_File.h \[5.2\] search
Save:SI\node\Macro_File.h \[5.3\] Group (不用)
Save:SI\node\Macro_File.h \[5.4\] version (不稳定, 不要用)
// 6. set
Save:SI\node\Macro_File.h \[6.1\] set:
Save:SI\node\Macro_File.h \[6.2\] Tools:
Save:SI\node\Macro_File.h \[6.3\] SVN BCmp git SI vim
// 7. 其他
Save:SI\node\Macro_File.h \[7.1\] 禅道 展讯 MTK
Save:SI\node\Macro_File.h \[7.2\] 参考标号:








[1.1] work--node
//1) 新加项目目录:
Save:SI\node\set\Macro_Set_Help.h
//2) MTK SPR:
//	MTK, modis
Save:SI\node\note\Macro_modis_MTK.h
Save:SI\node\note\Macro_Note_MTK.h
//	6531E, modis
Save:SI\node\note\Macro_modis_6531E.h
Save:SI\node\note\Macro_Note_6531E.h
//	7701, modis
Save:SI\node\note\Macro_Note_7701.h

//3) other:
Save:SI\node\Project\Macro_A_Nav.h


[1.2] Tmp
//1) Tmp File
Save:SI\node\Simple_CTRL_B.h
//2) Bcmp Tmp File
Save:SI\node\Tmp_Comp_1.h
Save:SI\node\Tmp_Comp_2.h
//3) 其他:
Save:SI\bak\Macro_tmp.h TmpNote
Save:SI\bak\Macro_code_tmp.h



[2.2] 未整理 未改
//1) 未整理:
Save:SI\bak\bak_mtk_lang.h	
//2) 未改:
Save:SI\bak\bak_mtk_weiti.h	


[2.3] bak文件:
Save:SI\bak\bak_20181114_tmp_B_old.h
Save:SI\bak\bak_20181123_git.h
	




[3.1] Cmd:
Save:SI\Cmd\clear_mtk__no_use
Save:SI\Cmd\dir__no_use
Save:SI\Cmd\other__no_use


[3.2] Cmd unzip:
Save:SI\Cmd_unzip\6531E\unzip_spr128.bat
Save:SI\Cmd_unzip\6531E\unzip_spr240_mini.bat

[3.3] Cmd 对比:
Save:SI\bak\bcmp\bmp_107_mk.bat





[4.1] Test:
Save:SI\node\Test\Macro_Tmp_Node_Test.h
Save:SI\node\Test\sbd_Tmp_Show.em
Save:SI\node\Test\Macro_Tmp_...
Save:SI\node\Test\...
Save:SI\node\Test\Tmp_Test.h
// F11功能测试:
Save:SI\node\Test\Macro_Tmp_Node_F11.h


[4.2] file list Test:
Save:SI\Cmd\file___no_use\...
Save:SI\Cmd\file___no_use\dir_getFile...
Save:SI\Cmd\file___no_use\dir_getFile time.bat
Save:SI\Cmd\file___no_use\dir_getFile_spr.bat
Save:SI\Cmd\cmd_getFileList.bat
Save:SI\Cmd_TXT\si_filelist.h


[4.3] Msg
//1) Nv Msg
Save:SI\node\set\Macro_Val.h
//2) Pic Path Msg
Save:SI\node\set\Show_Pic_dir.h
//3) Key Msg
Save:SI\node\Simple_CTRL_K.h




[5.1] rule
//1) rule 中文:
Save:SI\node\rule\Macro_Rule_Han_file.h
//2) rule LCD+CAM 文件跳转:
Save:SI\node\rule\Macro_Rule_Key_file_6531E.h
Save:SI\node\rule\Macro_Rule_Key_file_7701.h
Save:SI\node\rule\Macro_Rule_Key_file_6533.h
Save:SI\node\rule\Macro_Rule_Key_file_MTK.h

[5.2] search
//1) search 所有宏:
Save:SI\node\search\Macro_ALL_6531E.h
//2) search 9820e 所有文件:
Save:SI\node\search\Macro_ALL_9820e_JAVA.h

[5.3] Group (不用)
Save:SI\node\group\Macro_Group_Make_6531E.h

[5.4] version (不稳定, 不要用)
Save:SI\node\version\si_version_6531e.h



[6.1] set:
//1) Common set
Save:SI\node\set\Macro_Set.h
//	基础路径设置:
Save:SI\node\set\Macro_Set_Base.h
//2) windows system Path
Save:SI\node\Tmp_Common.h
//3) cur project
Save:SI\node\set\Macro_Set_Note.h
//set--utf8:
Save:SI\node\set\Macro_Set.h 63

[6.2] Tools:
Save:SI\node\note\Macro_Note_Tools.h

[6.3] SVN BCmp git SI vim
//1) SVN 配置:
Save:SI\node\ToolsMsg\Macro_SVN.h
C:\Users\Administrator\AppData\Roaming\Subversion\config
//2) BCmp 配置:
C:\Users\Administrator\AppData\Roaming\Scooter^Software\Beyond^Compare^4
//   BCmp:
Save:SI\node\ToolsMsg\Macro_Bcomp.h
//3) SecureCRT:
Save:SI\node\ToolsMsg\Macro_SecureCRT.h
//4) git:
Save:SI\node\ToolsMsg\Macro_git.h
//5) SI:
Save:SI\node\ToolsMsg\Macro_SI.h
//6) vim:
Save:SI\node\ToolsMsg\Macro_vim.h
Save:SI\node\ToolsMsg\Macro_MTK_FlashTool.h





[7.1] 禅道 展讯 MTK
//1) 禅道:
http://192.168.2.252/zentao/my/
//2) 展讯:
http://222.66.158.139:2008/cqweb/#
//3) MTK:
https://online.mediatek.com




[7.2] 参考标号:
//参考标号:
Save:SI\node\Test\Macro_Tmp_Node_Num.h



