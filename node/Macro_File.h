
// File List [Num][Ca]:
// 1. bak Cmd
Save:SI\node\Macro_File.h \[1.1\] bak文件:
Save:SI\node\Macro_File.h \[1.2\] Cmd:
Save:SI\node\Macro_File.h \[1.3\] Cmd unzip:
Save:SI\node\Macro_File.h \[1.4\] file list Test:
// 2. Macro MacroSBD Test
Save:SI\node\Macro_File.h \[2.1\] Macro path Set:
Save:SI\node\Macro_File.h \[2.2\] Test:
// 3. Project modis (Common)
Save:SI\node\Macro_File.h \[3.1\] 常用的编辑文件
Save:SI\node\Macro_File.h \[3.2\] 模拟器
// 4. Set
Save:SI\node\Macro_File.h \[4.1\] set:
Save:SI\node\Macro_File.h \[4.2\] Msg
Save:SI\node\Macro_File.h \[4.3\] Tmp File
Save:SI\node\Macro_File.h \[4.4\] rule
Save:SI\node\Macro_File.h \[4.5\] search
Save:SI\node\Macro_File.h \[4.6\] Group (不用)
Save:SI\node\Macro_File.h \[4.7\] version (不稳定, 不要用)
// 5. Tool
Save:SI\node\Macro_File.h \[5.1\] Tools:
Save:SI\node\Macro_File.h \[5.2\] SVN BCmp git SI vim
// 6. Other
Save:SI\node\Macro_File.h \[6.1\] 禅道 展讯 MTK
Save:SI\node\Macro_File.h \[6.2\] 未整理:
Save:SI\node\Macro_File.h \[6.3\] 未改:
	


/***********************************************************************/

[1.1] bak文件:
Save:SI\bak\bak_20181114_tmp_B_old.h
Save:SI\bak\bak_20181123_git.h
对比:
Save:SI\bak\bcmp\bmp_107_mk.bat

[1.2] Cmd:
Save:SI\Cmd\clear_mtk__no_use
Save:SI\Cmd\dir__no_use
Save:SI\Cmd\other__no_use
		
[1.3] Cmd unzip:
Save:SI\Cmd_unzip\6531E\unzip_spr128.bat
Save:SI\Cmd_unzip\6531E\unzip_spr240_mini.bat
	
[1.4] file list Test:
Save:SI\Cmd\file___no_use\...
Save:SI\Cmd\file___no_use\dir_getFile...
Save:SI\Cmd\file___no_use\dir_getFile time.bat
Save:SI\Cmd\file___no_use\dir_getFile_spr.bat
Save:SI\Cmd\cmd_getFileList.bat
Save:SI\Cmd_TXT\si_filelist.h

/***********************************************************************/

[2.1] Macro path Set:
Save:SI\MacroSBD\sbd_base.em


[2.2] Test:
Save:SI\node\Test\Macro_Tmp_Node_Test.h
Save:SI\node\Test\Macro_Tmp_...
Save:SI\node\Test\...
Save:SI\node\Test\Tmp_Test.h


/***********************************************************************/

[3.1] 常用的编辑文件
Save:SI\node\note\Macro_Note_MTK.h
Save:SI\node\note\Macro_Note_9820es.h
Save:SI\node\note\Macro_Note_6531E.h
	
[3.2] 模拟器:
Save:SI\node\Macro_modis_MTK.h
Save:SI\node\Macro_modis_6531E.h


/***********************************************************************/

[4.1] set:
//1) Common set
Save:SI\node\Macro_Set.h
//2) windows system Path
Save:SI\node\Tmp_Common.h
//3) cur project
Save:SI\node\Macro_Set_Note.h

[4.2] Msg
//1) Nv Msg
Save:SI\node\Macro_Val.h
//2) Pic Path Msg
Save:SI\node\Show_Pic_dir.h
//3) Key Msg
Save:SI\node\Simple_CTRL_K.h


[4.3] Tmp File
//1) Tmp File
Save:SI\node\Simple_CTRL_B.h
//2) Tmp File
Save:SI\node\Simple_CTRL_B_UTF8.h
//3) Bcmp Tmp File
Save:SI\node\Tmp_Comp_1.h
Save:SI\node\Tmp_Comp_2.h


[4.4] rule
//1) rule 中文:
Save:SI\node\rule\Macro_Rule_Han_file.h
//2) rule LCD+CAM 文件跳转:
Save:SI\node\rule\Macro_Rule_Key_file_6531E.h
Save:SI\node\rule\Macro_Rule_Key_file_7701.h
Save:SI\node\rule\Macro_Rule_Key_file_6533.h
Save:SI\node\rule\Macro_Rule_Key_file_MTK.h

[4.5] search
//1) search 所有宏:
Save:SI\node\search\Macro_ALL_6531E.h
//2) search 9820e 所有文件:
Save:SI\node\search\Macro_ALL_9820e_JAVA.h

[4.6] Group (不用)
Save:SI\node\group\Macro_Group_Make_6531E.h

[4.7] version (不稳定, 不要用)
Save:SI\node\version\si_version_6531e.h


/***********************************************************************/

[5.1] Tools:
Save:SI\node\note\Macro_Note_Tools.h

[5.2] SVN BCmp git SI vim
//1) SVN 配置:
Save:SI\node\ToolsMsg\Macro_SVN.h
C:\Users\Administrator\AppData\Roaming\Subversion\config
//2) BCmp 配置:
C:\Users\Administrator\AppData\Roaming\Scooter^Software\Beyond^Compare^4
//   BCmp:
Save:SI\node\ToolsMsg\Macro_Bcomp.h
//3) SecureCRT:
Save:SI\node\ToolsMsg\Macro_SecureCRT.h
//4) git:
Save:SI\node\ToolsMsg\Macro_git.h
//5) SI:
Save:SI\node\ToolsMsg\Macro_SI.h
//6) vim:
Save:SI\node\ToolsMsg\Macro_vim.h


/***********************************************************************/

[6.1] 禅道 展讯 MTK
//1) 禅道:
http://192.168.2.252/zentao/my/
//2) 展讯:
http://222.66.158.139:2008/cqweb/#
//3) MTK:
https://online.mediatek.com


[6.2] 未整理:
Save:SI\bak\bak_mtk_lang.h

[6.3] 未改:
Save:SI\bak\bak_mtk_weiti.h	

Save:SI\bak\Macro_tmp.h

Save:SI\bak\Macro_bak_mtk_weiti.h	
Save:SI\bak\Macro_bak_mtk_Bengali.h 


