...
/***********************************************************************/
快捷键[Ca]: 
ctrl+D->ctrl+a, s, e, f, g: 跳转至文件头/尾
ctrl+D->3: 查看隐藏注释, 如[ 3.3.3 ]
		  [ 3.3.3 ] 这个文件中表示未完成功能
ctrl+D->ctrl+D->1~9+26: 显示索引, 输入索引跳转至对应位置
F12->F1~F12: show help pop
F10->40条添加: 上一页下一页 ctrl+D->4
...
Macro_z_null.h 试打开--判断存在--用默认路径
ctrl+R/G->marco_中-》下一个标签

1 image->F9->file
2 ctrl+I->char. 在前加编吗
2 ctrl+T->根据首行是否居首, 删除注释
1 F9, 默认行尾文件名, java(用不上)
...
/************************** 可以用的快捷键 *****************************/
/***********************************************************************/
//[6D] ctrl+B共用 +U任用 +E通用; -D简用 -K化用 -N独用
//->B<-    //-->>E    //<-D->    //U<<--    //->K->    //<-N<-

这套宏:
带文本笔记功能, 笔记区分工程
笔记中可以打开exe、目录、文件+某关键字、make/vs/vs2008/py编译命令
  其他路径列表替换命令
笔记内带索引
运行svn, 对比批处理, 笔记内的copy批处理
宏--关键字跳转、添加跳转关系
宏--显示注释、添加宏注释
标号功能
单/多行文字批处理 添加前缀, 数字自动增加


用的比较多的功能:
F1, F2, F5, F6, F7, F9, F10, F11, F12 用的比较多
ctrl+D, T, E, Q, B, Back,  用的比较多
ctrl+N, U, I, C, 用的比较少
...
[1.1](未实现)F7 -> X -> 清除.mk注释  (未实现)
[1.1](未实现)F7 -> L -> build-查宏-显示在si  (未实现)
[1.1](未实现)F7 -> M -> build-查log   type app* |find  "SPEAK" > log.txt  (未实现)
[1.1](未实现)F7 -> F -> 批处理-当前路径文件名  (未实现)
[1.1](未实现) F7 -> H: last模式 最后一次命令：如svn添加文件
...
//ctrl+D->ctrl+D 显示默认索引
		  4. .mk .def .mak中 同F10
		  5. .em中 显示所有函数名
//[1.1](未实现)4*. 选择1整行+ 设置对齐默认值;ctrl+D--sr--up, down;ctrl+D--mk--search copy
//[1.1](未实现)3*. 选中, 注释     对齐
