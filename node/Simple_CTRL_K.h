...
当前文件快捷键[Ca]: 
1) 跳转至文件头/尾
	ctrl+D->ctrl+a, s, e, f, g
2) 显示未完成功能
	ctrl+D->ctrl+D, 弹窗按ok再按数字序号跳转至对应位置
3) 跳转至F1~F12的说明位置
	ctrl+D->F1~F12
4) 弹窗显示F1~F12的功能说明
	F12->F1~F12
5) 加这个, 弹窗默认显示数字, 如[1z.2n]
	[Num]
...
F7 -> B -> 切换--/F6功能: F6多工程文件对比, 设置对比工程
           增加功能: F1->F6 目录对比
F7 -> L -> 切换--/F6功能: F6关闭
F7 -> N -> 切换--/F6功能: F6服务器路径 //[3.1]后面增加提示切换ok
...
4 在em文件
//	显示Macro()+行号, 方便进一步编辑
5 在笔记文件, 文件名以"Macro_"、"Simple_CTRL_"开头, 同ctrl+D
	//ctrl+D->~ 显示帮助
	//ctrl+D->1 显示索引[a-z]+
	...
	如果记得[r]、[Cb]的功能, ctrl+D->r、ctrl+D->ctrl+B跳转不会显示弹窗, mtk笔记是这样
	为了不影响数字索引, 括号改成这样: \[fail\], 关键字也可以跳转
	//	1. \[1.1\]...
	//	6. \[1.6\]...
	//	   ...
	//	10. \[1.10\]...
	索引可能加要0: ctrl+D->ctrl+D->Msg->01, ctrl+D->ctrl+D->Msg->6, 没有60输6也可以
		
//[3.1](未实现) 40条后 添加提示: 上一页下一页 ctrl+D->4 5 6
