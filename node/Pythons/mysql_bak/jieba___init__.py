...
class Tokenizer(object):
    ...
    def initialize(self, dictionary=None):
        default_logger.debug("zfzzz initialize %s." % dictionary)
        if dictionary:
    ...
    def getFREQ(self, i_start=1, i_count=10):
        self.check_initialized()
        list_len = len(self.FREQ)
        default_logger.debug("zfzzz list_len %s." % list_len)
        default_logger.debug("zfzzz i_start %s." % i_start)
        default_logger.debug("zfzzz i_count %s." % i_count)
        i = 0
    ...
    #default_logger.debug("zfzzz item %s." % i)
                default_logger.debug("zfzzz item %s   %s." % (key, value))
                        
                sql = """INSERT INTO DICT(DTXT,DCNT)VALUES ('%s', %d)""" % (key, value)
                cursor.execute(sql)
        ...
        for blk in blocks:
            ...
            if re_han.match(blk):
                for word in cut_block(blk):
                    #default_logger.debug("test word %s." % word)
                    yield word
            else:
                #default_logger.debug("test word --err %s." % blk)
