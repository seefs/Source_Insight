...
def get_category_from_db(unique_list):
# 从数据库获取词性
    number_list = []
    ...
    try:
        for j in range(len(unique_list)):
            ...
            mysql_cursor.execute(sql_cx % (str(unique_str)))
...
def connect_content_from_db(concat_list):
# 从数据库获取词性
...
def get_single_code_from_db(unique_list):
# 单字模式:
#   按下标顺序分隔
#   为代码简单, 只处理常用
    index_dict = { 1:[[1]],
        ...
        for tmp_str in unique_list:
            ...
            for tmp_index_list in index_list:
                ...
                for tmp_index in tmp_index_list:
                    ...
                    # 查找词性+备注, 备注如:多义词词性列表
                    mysql_cursor.execute(sql_cx % (str(word_str)))
                    dcxn = 27
