...
class LoadData:
    ...
    def _add_head(self, input_data):
            self.max_seq_len = len(self.test_m1[0])
        data_cnt = 0
        if self.train_enable:
            self.train_h1 = [[i] for i in range(self.max_seq_len)]
            data_cnt += self.max_seq_len
        if self.test_enable:
            self.test_h1 = [[data_cnt + i] for i in range(self.max_seq_len)]
            data_cnt += self.max_seq_len
    ...
    def _load_data_from_csv(self):
        ...
        data_cnt = 1
        if self.train_enable:
            for i in range(len(self.train_t1)):
                self.train_t1[i][0] = '_' + str(data_cnt)
                data_cnt+=1
            for i in range(len(self.train_t2)):
                self.train_t2[i][0] = '_' + str(data_cnt)
                data_cnt+=1
        if self.test_enable:
            for i in range(len(self.test_t1)):
                self.test_t1[i][0] = '_' + str(data_cnt)
                data_cnt+=1
            for i in range(len(self.test_t2)):
                self.test_t2[i][0] = '_' + str(data_cnt)
                data_cnt+=1
    ...
    def _prepare_cn(self, df):
        ...
        with tqdm(total=df.shape[0], unit_scale=True) as pbar:
            for ndx, row in df.iterrows():
                ...
                tokens1 = [['_C', 0]] + self.token_chg.tokens_mode2(text=text1)
                tokens2 = [['_C', 0]] + self.token_chg.tokens_mode2(text=text2)
