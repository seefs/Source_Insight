DROP PROCEDURE IF EXISTS sp_sentence_path_update;

DELIMITER $$    
CREATE PROCEDURE sp_sentence_path_update()     
BEGIN    
    ...
    /* 声明游标 */  
    DECLARE myCursor CURSOR FOR SELECT CID,CCOUNT,CSTR FROM sentence_path_all;  
    ...
    myLoop: LOOP  
        ...
        SET @ncx_tmp:=(SELECT `DCXE` FROM `dict_utf8_update` WHERE `DTXT`= @nstr_tmp);
...
#            SELECT @nstrs_tmp;
            ...
            SET i=i+1;
        END WHILE;
        UPDATE `sentence_path_all` SET `NSTR`=@nstrs_tmp  WHERE CID=scid ;
