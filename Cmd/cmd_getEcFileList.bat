...
for /f "delims=" %%i   in ('dir  /b/a-d/s  %schDir%')  do (
	...
	if "!curpath!"=="" (
		...
		set "f="
	) else if "!curpath:~0,9!"==".settings" (
		set "f="
	) else if "!curpath:~0,4!"=="bin\" (
		set "f="
	) else if "!curpath:~0,4!"=="gen\" (
		set "f="
	) else if "!curpath:~0,5!"=="libs\" (
		set "f="
	) else if "!curpath:~0,9!"=="proguard\" (
		set "f="
    ...
    echo !curpath!%%~nxi | findstr .svn >nul || @echo !curpath!%%~nxi
	) 
